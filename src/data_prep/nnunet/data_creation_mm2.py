...
#
    for label_file in file_list[:max_items]:
        ...
        #
        if subdir == 'train':
            target_img_path = os.path.join(ddest, 'imagesTr', base_name + "_0000" + base_ext)
            target_label_path = os.path.join(ddest, 'labelsTr', img_file)
        else:
            print(subdir)
